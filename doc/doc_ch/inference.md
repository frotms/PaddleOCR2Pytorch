...
PP-StructureV3 是文档结构化解析系统，包含布局检测、OCR、表格识别、公式识别、印章检测、文档预处理共 8 个模型。
...
**管道流程**：输入图片 → [doc_ori] → [UVDoc] → 布局检测 + 全局OCR → [表格/公式/印章] → 阅读顺序恢复 → Markdown/JSON/可视化
