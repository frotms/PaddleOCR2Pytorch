...
"""
PyTorch port of UniMERNet decoder head.
Reference: https://github.com/opendatalab/UniMERNet/blob/main/unimernet/models/unimernet/configuration_unimernet_decoder.py
"""
...
class ModelOutput(OrderedDict):
    """Base class for all model outputs as dataclasses."""
...
class AttentionMaskConverter:
    ...
    def __init__(self, is_causal: bool, sliding_window=None):
        ...
        if self.sliding_window is not None and self.sliding_window <= 0:
            raise ValueError(
                f"Make sure that when passing `sliding_window` that its value is a strictly positive integer, "
    ...
    def to_4d(
        ...
        if (input_shape[-1] > 1 or self.sliding_window is not None) and self.is_causal:
            if key_value_length is None:
                raise ValueError(
                    "This attention mask converter is causal. Make sure to pass `key_value_length` "
                    "to correctly create a causal mask."
                )
...
# =============================================================================
# MBart Positional Embedding
# =============================================================================

class MBartLearnedPositionalEmbedding(nn.Embedding):
    """Learned positional embeddings up to a fixed maximum size."""
...
class myLayerNorm(nn.Module):
    """Custom Layer Normalization matching Paddle implementation."""

    def __init__(self, num_channels, eps=1e-5, affine=True, drop_block=None):
        super().__init__()
        self.eps = eps
...
class MBartDecoderWrapper(MBartPreTrainedModel):
    """Wrapper class for correct pretrained checkpoint loading."""
...
# =============================================================================
# Self-attention block and Counting Decoder
# =============================================================================

class SelfAttentionBlock(nn.Module):
    """Self-attention block with multi-head attention and layer norm."""

    def __init__(self, embed_size, num_heads, is_export=False):
        super().__init__()
...
class SeqCountingDecoder(nn.Module):
    """Sequence counting decoder with multi-head attention and FFN."""
...
# =============================================================================
# Custom MBart Decoder and CausalLM (with counting)
# =============================================================================

class CustomMBartDecoder(MBartDecoder):
    """Custom MBartDecoder with counting context weight."""
...
class CustomMBartForCausalLM(MBartForCausalLM):
    """Custom MBart model with counting decoder."""
...
# =============================================================================
# Multihead Attention (custom)
# =============================================================================

def _in_projection_packed(q, k, v, w, b=None, is_export=False):
    """Compute in-projection for multi-head attention."""
    E = q.shape[-1]
    if k is v:
        if q is k:
            # self-attention
            ...
            if is_export:
                ...
                proj = proj.unsqueeze(0).permute([3, 1, 2, 0, 4]).squeeze(-2).contiguous()
            else:
                proj = (
...
class MyMultiheadAttention(nn.Module):
    """Custom multi-head attention matching Paddle implementation."""
    ...
    def __init__(
        ...
        if self._qkv_same_embed_dim is False:
            ...
            if dtype is None:
                dtype = torch.float32
            self.in_proj_weight = nn.Parameter(torch.empty(3 * embed_dim, embed_dim, dtype=dtype))
            self.q_proj_weight = None
            self.k_proj_weight = None
            self.v_proj_weight = None

        if bias:
            self.in_proj_bias = nn.Parameter(torch.empty(3 * embed_dim, dtype=dtype))
...
class LogitsProcessorList(list):
    """A list of logits processors applied sequentially."""
...
class ForcedEOSTokenLogitsProcessor(object):
    """Forces EOS token at max_length position."""
...
class UniMERNetHead(nn.Module):
    """UniMERNet decoder head.

    Args:
        max_new_tokens: Maximum number of new tokens to generate.
        decoder_start_token_id: ID of the token that starts decoding.
        ...
        decoder_ffn_dim: Dimension of the decoder's FFN.
        decoder_layers: Number of decoder layers.
        is_export: Flag for model export.
        length_aware: Flag to enable length-aware mechanisms.
    """
    ...
    def _get_decoder_start_token_id(
        ...
        decoder_start_token_id = (
            ...
            else getattr(self.generation_config, 'decoder_start_token_id', 0)
        )
        bos_token_id = (
            bos_token_id
            if bos_token_id is not None
            else getattr(self.generation_config, 'bos_token_id', 0)
        )
        if decoder_start_token_id is not None:
    ...
    def prepare_inputs_for_generation_mbart(
        ...
        **kwargs,
        ...
        if attention_mask is None:
            attention_mask = torch.ones(input_ids.shape, dtype=torch.int64, device=input_ids.device)
    ...
    def generate(self, model_kwargs):
        ...
        unfinished_sequences = torch.ones(batch_size, dtype=torch.int64, device=input_ids.device)
