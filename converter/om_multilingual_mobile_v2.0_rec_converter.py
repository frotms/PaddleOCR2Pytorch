...
class MultilingualV20RecConverter(BaseOCRV20):
    ...
    def load_paddle_weights(self, paddle_weights):
        ...
        # [print('pytorch: ', k, v.shape) for k,v in self.net.state_dict().items()];exit()
...
if __name__ == '__main__':
    ...
    if args.dst_model_path is not None:
        ...
        save_name = 'om_{}infer.pth'.format(os.path.basename(os.path.dirname(paddle_pretrained_model_path))[:-5])
    ...
    torch.onnx.export(converter.net, dummy_input, 'om_{}infer.onnx'.format(os.path.basename(os.path.dirname(paddle_pretrained_model_path))[:-5]), opset_version=11,
